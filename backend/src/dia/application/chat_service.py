"""聊天服务层 (application) — Multi-Agent SSE 编排用例

职责: 意图分类 → 会话状态构建 → LangGraph 流式执行 → 事件转发 → 报告分段
     → 会话持久化 → 历史结论落盘 → token 汇总。不依赖 FastAPI router。
依赖方向: api/v1/chat.py (router) → 本模块 → graph/agents/infrastructure
"""
import json
import logging
...
from dia.infrastructure.persistence.sessions import get_session, save_session
...
async def stream_chat(
    ...
    """Multi-Agent Supervisor 流式执行 — 生成 SSE 数据行.

    Args:
        is_disconnected: 客户端断连检测回调 (由 router 注入 request.is_disconnected,
            服务层不依赖 FastAPI Request)
    """
