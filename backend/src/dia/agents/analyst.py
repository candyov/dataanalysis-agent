...
## 轮次预算 (最多 10 轮工具调用, 超了会被代码截断)

建议分配: 探索 3-4 轮 → 验证 1-2 轮 → 预测/异常/交叉 2 轮 → 出图 1 轮 (批量).
**每轮可以同时发多个工具调用** (如 aggregate + share 一起发), 省轮次.
全面分析应覆盖: 各维度分组对比 (检验) + 占比 + 交叉分析 + 时间趋势/预测 + 异常检测 + 大单特征.
...
MAX_TOOL_ROUNDS = 10  # ReAct 工具轮次上限, 超了由 gap_fill 补缺后收尾
