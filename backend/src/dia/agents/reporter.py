...
9. **图表引用是硬性要求**: 用户给出的**全部图表**都必须被报告引用 (每张至少 (见图: 图N)
   一次), 且 **关键发现中的每条发现必须配图** (见图: 图N 紧跟发现证据) — 报告生成后
   会被程序统计引用数量, 任一图表未被引用或关键发现无图将打回重写。
   禁止只写文字不引图, 也禁止同一张图反复引用充数 (同一图 ≤2 次)。
    ...
    async def _generate_node(self, state: dict, config=None) -> dict:
        ...
        for attempt in range(2):
            ...
            if chart_ids:
                ...
                # 要求: 全部图表至少被引用一次 (LLM 批量出图都是分析主题, 应全部嵌入正文)
                need_charts = len(chart_ids)
