...
| 数据库 | MySQL / PostgreSQL / SQLite(连接默认只读)+ CSV / Excel 上传 |
| LLM | DeepSeek(默认，兼容任意 OpenAI 协议模型，支持多模型档案运行时切换) |
...
backend/src/dia/
  main.py            # FastAPI 入口
  api/
    v1/              # HTTP 路由: chat(SSE) / datasources / models / settings (薄 router)
    schemas/         # 请求/响应契约 (ChatRequest 等)
    middleware/      # 鉴权 / 链路追踪
  application/       # ★ 服务层: chat_service (SSE 编排用例, 不依赖 FastAPI)
